    ...
    <nav class="relative flex items-center justify-between sm:h-10 lg:justify-start" aria-label="Global">
        <div class="flex items-center flex-grow flex-shrink-0 lg:flex-grow-0">
            <div class="flex flex-row justify-between w-full md:w-auto">
                <p class="font-bold text-rose-600 text-xl"> MyMenu </p>
                ...
                focus:ring-2 focus:ring-inset focus:ring-rose-600"
                    ...
                    <span class="ml-2 text-sm font-medium text-gray-700 group-hover:text-gray-800">{{shopping_cart.length}}</span>
                </div>
            </div>
        </div>
    </nav>
...
import {inject} from "vue";
    ...
    const shopping_cart = inject('shopping_cart')
